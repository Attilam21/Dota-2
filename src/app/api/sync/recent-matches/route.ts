...
export async function GET(req: Request) {
  ...
  try {
    const recent = await fetchFromOpenDota<RecentMatch[]>(
      `/players/${playerId}/recentMatches`,
    )
    // TEMP LOG: payload grezzo per audit
    console.log(
      'DOTA2_RAW_RESPONSE',
      JSON.stringify(recent?.slice?.(0, 3) ?? recent, null, 2),
    )
    ...
    if (error) {
      console.error('Supabase upsert error (matches_digest)', error.message)
    ...
    return NextResponse.json({ ok: true, count: rows.length, playerId })
  } catch (e: any) {
    // Non-bloccante: fallback per il frontend
    console.error('OpenDota sync error', e?.message ?? e)
    return NextResponse.json({
      ok: false,
      source: 'opendota',
      status: e?.status ?? 0,
