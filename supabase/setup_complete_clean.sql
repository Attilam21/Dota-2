-- SCRIPT COMPLETO SETUP SUPABASE DOTA-2 DASHBOARD
-- Copia tutto questo script e incollalo nel SQL Editor di Supabase
...
-- 2. AGGIUNTA COLONNE A MATCHES_DIGEST
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'matches_digest' 
                 AND column_name = 'is_eligible_for_coaching') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'matches_digest' 
                 AND column_name = 'included_in_coaching') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'matches_digest' 
                 AND column_name = 'match_date') THEN
    ALTER TABLE matches_digest ADD COLUMN match_date TIMESTAMPTZ;
  END IF;
END $$;

-- 3. AGGIUNTA COLONNA A PLAYERS_DIGEST
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
...
-- 11. FUNZIONE MATCH_DATE
CREATE OR REPLACE FUNCTION set_match_date_from_start_time()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.match_date IS NULL THEN
    IF NEW.start_time IS NOT NULL THEN
      IF pg_typeof(NEW.start_time) = 'bigint'::regtype OR pg_typeof(NEW.start_time) = 'integer'::regtype THEN
        NEW.match_date = to_timestamp(NEW.start_time::bigint);
      ELSIF pg_typeof(NEW.start_time) = 'text'::regtype OR pg_typeof(NEW.start_time) = 'character varying'::regtype THEN
        BEGIN
          NEW.match_date = NEW.start_time::timestamptz;
