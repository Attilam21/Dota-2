// Check counts for FZTH aggregates for demo player
...
async function main() {
  ...
  if (!playerId) {
    console.log(JSON.stringify({ hasFzthPlayer: false }, null, 2))
    return
  }
  const [{ count: stats }, { count: hero }, { count: prog }, { count: ach }] = await Promise.all([
